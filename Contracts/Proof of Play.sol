...
interface IBattledog {
    ...
    function getPlayers(uint256 _tokenId) external view returns (Player[] memory);
...
contract ProofOfPlay is Ownable, ReentrancyGuard {
    ...
    function getMinerData(uint256 _tokenId) public nonReentrant {
        IBattledog.Player[] memory players = IBattledog(battledogs).getPlayers(_tokenId);
    ...
    function mineGAME(uint256[] calldata _nfts) public nonReentrant {
        ...
        for (uint256 a = 0; a < _nfts.length; a++) {
            ...
            // Update ActiveMiners array
            IBattledog.Player[] memory players = IBattledog(battledogs).getPlayers(tokenId);
            ActiveMiners[tokenId] = players[tokenId];
